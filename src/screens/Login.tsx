import { User as UserIcon, Lock, ArrowRight, Chrome, Facebook, Mail } from 'lucide-react';
...
export function Login({ goTo }: { goTo: GoToType }) {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // No need to call goTo('home'), the onAuthStateChanged in App.tsx will handle it
    } catch (e: any) {
      console.error(e);
      if (e.code === 'auth/operation-not-allowed') {
  ...
  const handleFacebookLogin = async () => {
    try {
      ...
      if (e.code === 'auth/operation-not-allowed') {
        ...
        alert('Login with Facebook failed. Please try again.');
      }
    }
  };

  return (
    <div className="min-h-full bg-gradient-to-br from-brand-blue to-brand-teal px-6 py-12 flex flex-col justify-center items-center">
      <div className="w-full max-w-sm">
        <div className="text-center mb-10">
          <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg text-[40px] leading-none">
            🩺
          </div>
          <h1 className="text-3xl font-black text-white font-nunito mb-2">Welcome Back</h1>
          <p className="text-white/80 text-sm">Sign in to book appointments quickly</p>
        </div>

        <div className="bg-white rounded-[24px] p-6 shadow-xl w-full">
          ...
          <div className="mt-8 text-center border-t border-slate-100 pt-5">
            <p className="text-sm border-slate-200 text-text-muted">
              ...
              <button onClick={() => goTo('signup')} className="text-brand-teal font-bold">Sign Up</button>
            </p>
          </div>
        </div>
      </div>
